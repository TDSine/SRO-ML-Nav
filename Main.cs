...
namespace RSBot.ML.Views
{
    ...
    public partial class Main : UserControl
    {
        ...
        public class Server
        {
                        ...
                        using (connectedTcpClient = tcpListener.AcceptTcpClient())
                        {
                            using (NetworkStream stream = connectedTcpClient.GetStream())
                            {
                                ...
                                while ((length = stream.Read(bytes, 0, bytes.Length)) != 0)
                                {
                                    ...
                                    MoveCharacter(clientMessage.Trim());
                                }
                            }
                        }
                    }
            ...
            {
                // Assuming that Game.Player.Movement.Source returns a reference type,
                // directly modifying position will affect the original source object.
                // If it's a value type, ensure you're updating the Source object accordingly.
                ...
                switch (command.ToUpper())
                {
                    case "UP":
                        // Move character up
                        position.YOffset -= 10; // Assuming negative Y is up.
                        break;
                    case "DOWN":
                        // Move character down
                        position.YOffset += 10; // Assuming positive Y is down.
                        break;
                    case "LEFT":
                        // Move character left
                        position.XOffset -= 10; // Assuming negative X is left.
                        break;
                    case "RIGHT":
                        // Move character right
                        position.XOffset += 10; // Assuming positive X is right.
                        break;
                    default:
                        Console.WriteLine("Invalid command received: " + command);
                        return; // Exit the method if the command is not valid.
                }

                // After modifying the position based on the command, move the player.
                Game.Player.MoveTo(position, false);

                // ... existing code to interpret and execute command
                string response = $"{Game.Player.Movement.Source.X},{Game.Player.Movement.Source.Y}";
            ...
            private void SendResponse(string message)
            {
                ...
                {
                    Console.WriteLine("Error in sending response: " + ex.Message);
                }
            }
        ...
        private void comboViewType_SelectedIndexChanged(object sender, EventArgs e)
        {
